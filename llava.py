...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: LLaMAConf) -> None:
        ...
        if config.learning_block:
            ...
            self.img_enc = nn.Linear(1280, 512)
            self.img_dec = nn.Linear(512, config.n_embd)
    ...
    def forward(
        ...
        if self.learning_block:
            ...
            img_enc = self.img_enc(img_enc)
            img_dec = self.img_dec(img_enc)
            
            _v = self.learning_block_enc(x)
            _v = self.learning_block_dec(_v)
            v = v * (1 - self.influnce) + _v * self.influnce + img_dec * self.influnce
